...
int main(int argc, char* argv[]) {
  ...
  auto la = connection->getLocalUser()->getAudienceLatencyLevel();
  AG_LOG(INFO, "Get getDefaultAudienceLatencyLevel %d", la);  
  ...
  if (options.remoteUserId.empty()) {
    ...
    connection->getLocalUser()->setAudienceLatencyLevel(agora::rtc::AUDIENCE_LATENCY_LEVEL_LOW_LATENCY);
...
auto la1 = connection->getLocalUser()->getAudienceLatencyLevel();
  AG_LOG(INFO, "Get getAudienceLatencyLevel %d", la1);  
