...
#define DEFAULT_REDIS_IP "127.0.0.1:6379"
#define DEFAULT_REDIS_QUEUE "sample_queue"
...
struct SampleOptions {
  ...
  int adMode = 1;
  std::string redisIp = DEFAULT_REDIS_IP;
  std::string redisQueue = DEFAULT_REDIS_QUEUE;
  struct {
...
static void SampleSendAudioTask(
  ...
  while (!exitFlag) {
    ...
    if(reset) {
        audioAdFileParser->setFileParseRestart();
        reset = false;
      }
...
static void SampleSendVideoH264Task(
  ...
  while (!exitFlag) {
    ...
    if (isEnd) {
      adMode = false;
      adFileParser->setFileParseRestart();
      reset = true;
      printf("end ad video \n");
    }
...
static void SampleReadRedisTask(
  const SampleOptions& options, bool& exitFlag
) {
  std::string url = "tcp://";
  ...
  printf("redis thread\n");
  while (!exitFlag) {
    auto command = redis.lpop(options.redisQueue);
    if (command) {
      if( *command == "switch") {
        adMode = true;
      }
...
int main(int argc, char* argv[]) {
  ...
  optParser.add_long_opt("redisIp", &options.redisIp, "The redis server IP");
  optParser.add_long_opt("redisQueue", &options.redisQueue, "Redis Queue name");
...
// Create local user observer to monitor intra frame request
  ...
  AG_LOG(INFO, "READY FOR processing Redis commands!");
