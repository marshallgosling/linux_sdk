...
bool HelperAacFileParser::getEndOfFileTag()
{
	return end_of_file_;
}

void HelperAacFileParser::setOnce(bool once)
{
	only_once_ = once;
}
...
std::unique_ptr<HelperAudioFrame> HelperAacFileParser::getAudioFrame(int frameSizeDuration) {
  ...
  if (data_offset_ + ADTS_HEADER_SIZE > data_size_) {
    AG_LOG(INFO, "End of audio file, offset:%d, size:%d", data_offset_, data_size_);
    end_of_file_ = true;
    if(!only_once_) data_offset_ = 0;
  }
  ...
  audioFrameInfo.numberOfChannels = 2;
